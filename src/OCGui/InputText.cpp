...
namespace OCGui
{
    ...
    static bool InputTextEx(const char* label, char* buf, int buf_size, const Rect& rect, ImGuiInputTextFlags flags, ImGuiTextEditCallback callback, void* user_data)
    {
        ...
        ImVec2 size = ImGui::CalcItemSize(rect.GetSize()  - style.FramePadding*2.0f, ImGui::CalcItemWidth(), is_multiline ? ImGui::GetTextLineHeight() * 8.0f : label_size.y); // Arbitrary default of 8 lines high for multi-line
        ImGui::SetCursorPos(rect.GetPos());
        ...
        if (is_multiline)
        {
            ...
            // Draw border
            window->DrawList->AddRect(frame_bb.Min - Vec2(1,1), frame_bb.Max, window->Color(ImGuiCol_Border), style.FrameRounding);
        ...
        const bool focus_requested = ImGui::FocusableItemRegister(window, g.ActiveId == id, (flags & (ImGuiInputTextFlags_CallbackCompletion|ImGuiInputTextFlags_AllowTabInput)) == 0);    // Using completion callback disable keyboard tabbing
        ...
        if (g.ActiveId == id)
        {
            ...
            else if (IsKeyPressedMap(ImGuiKey_Escape))                              { ImGui::SetActiveID(0); cancel_edit = true; }
        ...
        if (!is_multiline)
            ImGui::RenderFrame(frame_bb.Min, frame_bb.Max, window->Color(ImGuiCol_FrameBg), true, style.FrameRounding);
        ...
        if (g.ActiveId == id || (edit_state.Id == id && is_multiline && g.ActiveId == draw_window->GetID("#SCROLLY")))
        {
            ...
            {
                draw_window->DrawList->AddLine(cursor_screen_pos + ImVec2(0.0f,-g.FontSize+0.5f), cursor_screen_pos + ImVec2(0.0f,-1.5f), window->Color(ImGuiCol_TextCursor));
                draw_window->DrawList->AddLine(cursor_screen_pos + ImVec2(1.0f,-g.FontSize+0.5f), cursor_screen_pos + ImVec2(1.0f,-1.5f), window->Color(ImGuiCol_TextCursor));
            }
...
//            ImGui::RenderText(ImVec2(frame_bb.Max.x + style.ItemInnerSpacing.x, frame_bb.Min.y + style.FramePadding.y), label);
    ...
    bool InputText(const String& label, const Rect& rect, char* buf, int buf_size)
    {
        ...
        bool ret = InputTextEx(label.c_str(), buf, (int)buf_size, rect, flags, nullptr, nullptr);
        return ret;
    }
    
    bool InputTextMultiline(const String& label, const Rect& rect, char* buf, int buf_size)
    {
        int flags = ImGuiInputTextFlags_Multiline;
        bool ret = InputTextEx(label.c_str(), buf, (int)buf_size, rect, flags, nullptr, nullptr);
        return ret;
    }
    ...
    namespace Layout
    {
        static bool InputTextEx(const char* label, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiTextEditCallback callback, void* user_data)
        {
            ...
            ImVec2 label_size = ImGui::CalcTextSize(label, NULL, true);
            ImVec2 size = ImGui::CalcItemSize(size_arg - style.FramePadding * 2.0f, ImGui::CalcItemWidth(), is_multiline ? ImGui::GetTextLineHeight() * 8.0f : label_size.y); // Arbitrary default of 8 lines high for multi-line
            const ImRect frame_bb(window->DC.CursorPos, window->DC.CursorPos + size + style.FramePadding*2.0f);
            const ImRect total_bb(frame_bb.Min, frame_bb.Max + ImVec2(label_size.x > 0.0f ? (style.ItemInnerSpacing.x + label_size.x) : 0.0f, 0.0f));
            ...
            if (g.ActiveId == id || (edit_state.Id == id && is_multiline && g.ActiveId == draw_window->GetID("#SCROLLY")))
            {
                ...
                ImVec2 cursor_screen_pos = render_pos + cursor_offset - render_scroll;
                bool cursor_is_visible = (g.InputTextState.CursorAnim <= 0.0f) || fmodf(g.InputTextState.CursorAnim, 1.20f) <= 0.80f;
                if (cursor_is_visible && !(flags & ImGuiInputTextFlags_ReadOnly))
                {
                    draw_window->DrawList->AddLine(cursor_screen_pos + ImVec2(0.0f,-g.FontSize+0.5f), cursor_screen_pos + ImVec2(0.0f,-1.5f), window->Color(ImGuiCol_TextCursor));
                    draw_window->DrawList->AddLine(cursor_screen_pos + ImVec2(1.0f,-g.FontSize+0.5f), cursor_screen_pos + ImVec2(1.0f,-1.5f), window->Color(ImGuiCol_TextCursor));
                }
        ...
        bool InputText(const String& label, const Size& size, char* buf, int buf_size, ImGuiInputTextFlags flags)
        {
            IM_ASSERT(!(flags & ImGuiInputTextFlags_Multiline)); // call InputTextMultiline()
            bool ret = InputTextEx(label.c_str(), buf, (int)buf_size, size, flags, nullptr, nullptr);
            return ret;
        }
        
        bool InputTextMultiline(const String& label, const Size& size, char* buf, int buf_size, ImGuiInputTextFlags flags)
        {
            bool ret = InputTextEx(label.c_str(), buf, (int)buf_size, size, flags | ImGuiInputTextFlags_Multiline, nullptr, nullptr);
            return ret;
        }
    }
...
////        const ImGuiStyle& style = g.Style;
////        
////        ImVec2 pos = m_position + style.FramePadding * 2.0f;
////        ImVec2 siz = m_size - style.FramePadding * 4.0f;
////        bndMultilineTextField(ImGui::GetNVGcontext(), pos.x, pos.y, m_renderPos.x, m_renderPos.y, siz.x, siz.y, 0, m_state, -1, m_buffer, m_carotPos.x, m_carotPos.y);
//    }
//    
...
//        char* buf = m_buffer;
//        int buf_size = m_bufferSize;
//        void* user_data = NULL;
//        ImGuiTextEditCallback callback = NULL;
//        
//        std::cout<< "DCB: " << window->DC.CursorPos.x << "," << window->DC.CursorPos.y << std::endl;
//        
...
//        
//        ImVec2 label_size = ImGui::CalcTextSize(label, NULL, true);
//        ImVec2 correctSize = CalcItemSize(size, CalcItemWidth(), is_multiline ? ImGui::GetTextLineHeight() * 8.0f : label_size.y); // Arbitrary default of 8 lines high for multi-line
//        const ImRect frame_bb(window->DC.CursorPos, window->DC.CursorPos + correctSize + style.FramePadding*2.0f);
//        const ImRect total_bb(frame_bb.Min, frame_bb.Max + ImVec2(label_size.x > 0.0f ? (style.ItemInnerSpacing.x + label_size.x) : 0.0f, 0.0f));
//        
//        m_position = total_bb.Min;
//        m_size = total_bb.Max - total_bb.Min;
//        
//        ImGuiWindow* draw_window = window;
//        if (is_multiline)
//        {
//            RenderFrame(frame_bb.Min, frame_bb.Max, ImGui::ColorConvertFloat4ToU32(ImVec4(0,0,0,1)));
//            ImGui::BeginGroup();
//            if (!ImGui::BeginChildFrame(id, frame_bb.GetSize()))
//            {
//                ImGui::EndChildFrame();
...
//            draw_window->DC.CursorPos += style.FramePadding;
//            correctSize.x -= draw_window->ScrollbarSizes.x;
//        }
//        else
//        {
...
//                        {
//                            // Insert character if they pass filtering
//                            if (!InputTextFilterCharacter(&c, m_flags, callback, user_data))
//                                continue;
//                            edit_state.OnKeyPressed((int)c);
//                        }
//                }
...
//                            if (c == 0)
//                                break;
//                            if (c >= 0x10000 || !InputTextFilterCharacter(&c, m_flags, callback, user_data))
//                                continue;
//                            clipboard_filtered[clipboard_filtered_len++] = (ImWchar)c;
//                        }
//                        clipboard_filtered[clipboard_filtered_len] = 0;
...
//            ImGui::PushStyleColor(ImGuiCol_BorderShadow, ImVec4(23.0/255.0,27.0/255.0,32.0/255.0,1));
//            ImGui::PushStyleColor(ImGuiCol_Border, ImVec4(23.0/255.0,27.0/255.0,32.0/255.0,1));
...
//        ImVec2 render_pos = is_multiline ? draw_window->DC.CursorPos : frame_bb.Min + style.FramePadding;
//        
//        ImVec4 clip_rect(frame_bb.Min.x, frame_bb.Min.y, frame_bb.Min.x + correctSize.x + style.FramePadding.x*2.0f, frame_bb.Min.y + correctSize.y + style.FramePadding.y*2.0f);
//        ImVec2 text_size(0.f, 0.f);
//        if (g.ActiveId == id || (edit_state.Id == id && is_multiline && g.ActiveId == draw_window->GetID("#SCROLLY")))
//        {
//            edit_state.CursorAnim += g.IO.DeltaTime;
...
//                // Calculate text height
//                if (is_multiline)
//                    text_size = ImVec2(correctSize.x, line_count * g.FontSize);
//            }
//            
//            // Scroll
//            if (edit_state.CursorFollow)
...
//                {
//                    const float scroll_increment_x = correctSize.x * 0.25f;
//                    if (cursor_offset.x < edit_state.ScrollX)
//                        edit_state.ScrollX = ImMax(0.0f, cursor_offset.x - scroll_increment_x);
//                    else if (cursor_offset.x - correctSize.x >= edit_state.ScrollX)
//                        edit_state.ScrollX = cursor_offset.x - correctSize.x + scroll_increment_x;
//                }
//                else
//                {
//                    edit_state.ScrollX = 0.0f;
...
//                    if (cursor_offset.y - g.FontSize < scroll_y)
//                        scroll_y = ImMax(0.0f, cursor_offset.y - g.FontSize);
//                    else if (cursor_offset.y - correctSize.y >= scroll_y)
//                        scroll_y = cursor_offset.y - correctSize.y;
//                    draw_window->DC.CursorPos.y += (draw_window->Scroll.y - scroll_y);   // To avoid a frame of lag
//                    draw_window->Scroll.y = scroll_y;
//                    render_pos.y = draw_window->DC.CursorPos.y;
//                }
...
//            const char* buf_end = NULL;
//            if (is_multiline)
//                text_size = ImVec2(correctSize.x, InputTextCalcTextLenAndLineCount(buf, &buf_end) * g.FontSize); // We don't need width
//            draw_window->DrawList->AddText(g.Font, g.FontSize, render_pos, draw_window->Color(ImGuiCol_Text), buf, buf_end, 0.0f, is_multiline ? NULL : &clip_rect);
//        }
//        
//        if (is_multiline)
...
////        const bool is_editable = (m_flags & ImGuiInputTextFlags_ReadOnly) == 0;
////        
////        ImVec2 pos = position + style.FramePadding * 2.0f;
////        ImVec2 correctSize = (size - style.FramePadding * 4.0f).NonNegative();
////        
////        ImVec2 label_size = ImGui::CalcTextSize(m_label.c_str(), NULL, true);
////        ImVec2 actualSize = CalcItemSize(correctSize, CalcItemWidth(), is_multiline ? ImGui::GetTextLineHeight() * 8.0f : label_size.y); // Arbitrary default of 8 lines high for multi-line
////        const ImRect frame_bb(pos, pos + actualSize);
...
////            draw_window = GetCurrentWindow();
////            draw_window->DC.CursorPos += style.FramePadding;
////            actualSize.x -= draw_window->ScrollbarSizes.x;
////        }
////        else
////        {
////            ItemSize(total_bb, style.FramePadding.y);
...
////            
////            // Edit in progress
////            const float mouse_x = 0.975 * ((g.IO.MousePos.x - frame_bb.Min.x - style.FramePadding.x) + edit_state.ScrollX);
////            const float mouse_y = (is_multiline ? (g.IO.MousePos.y - draw_window->DC.CursorPos.y - style.FramePadding.y) : (g.FontSize*0.5f));
////            
////            if (select_all || (hovered && io.MouseDoubleClicked[0]))
////            {
...
////        ImVec2 render_pos = is_multiline ? draw_window->DC.CursorPos : frame_bb.Min + style.FramePadding;
////        
//////        ImVec4 clip_rect(frame_bb.Min.x, frame_bb.Min.y, frame_bb.Min.x + actualSize.x + style.FramePadding.x*2.0f, frame_bb.Min.y + actualSize.y + style.FramePadding.y*2.0f);
////        ImVec2 text_size(0.f, 0.f);
////        if (g.ActiveId == id || (edit_state.Id == id && is_multiline && g.ActiveId == draw_window->GetID("#SCROLLY")))
////        {
////            edit_state.CursorAnim += g.IO.DeltaTime;
...
////                // Calculate text height
////                if (is_multiline)
////                    text_size = ImVec2(actualSize.x, line_count * g.FontSize);
////            }
////            
////            // Scroll
////            if (edit_state.CursorFollow)
...
//////                if (!(m_flags & ImGuiInputTextFlags_NoHorizontalScroll))
//////                {
//////                    const float scroll_increment_x = actualSize.x * 0.25f;
//////                    if (cursor_offset.x < edit_state.ScrollX)
//////                        edit_state.ScrollX = ImMax(0.0f, cursor_offset.x - scroll_increment_x);
//////                    else if (cursor_offset.x - actualSize.x >= edit_state.ScrollX)
//////                        edit_state.ScrollX = cursor_offset.x - actualSize.x + scroll_increment_x;
//////                }
//////                else
//////                {
//////                    edit_state.ScrollX = 0.0f;
...
////                    if (cursor_offset.y - g.FontSize < scroll_y)
////                        scroll_y = ImMax(0.0f, cursor_offset.y - g.FontSize);
////                    else if (cursor_offset.y - (actualSize.y - 4 * style.FramePadding.y) >= scroll_y)
////                        scroll_y = cursor_offset.y - (actualSize.y - 4 * style.FramePadding.y);
////                    draw_window->DC.CursorPos.y += (draw_window->Scroll.y - scroll_y);   // To avoid a frame of lag
////                    draw_window->Scroll.y = scroll_y;
////                    render_pos.y = draw_window->DC.CursorPos.y;
////                }
...
////            {
////                m_state = BND_ACTIVE;
////                m_carotPos = Vec2(edit_state.StbState.select_start, edit_state.StbState.select_end);
////            }
...
////            {
////                m_state = BND_ACTIVE;
////                m_carotPos = Vec2(edit_state.StbState.cursor, edit_state.StbState.cursor);
////            }
////            
////            m_renderPos = render_pos - render_scroll;
////            
////            // Draw blinking cursor
////            ImVec2 cursor_screen_pos = render_pos + cursor_offset - render_scroll;
////            //            bool cursor_is_visible = (g.InputTextState.CursorAnim <= 0.0f) || fmodf(g.InputTextState.CursorAnim, 1.20f) <= 0.80f;
...
////            const char* buf_end = NULL;
////            if (is_multiline)
////                text_size = ImVec2(actualSize.x, InputTextCalcTextLenAndLineCount(m_buffer, &buf_end) * g.FontSize); // We don't need width
////            // draw_window->DrawList->AddText(g.Font, g.FontSize, render_pos, draw_window->Color(ImGuiCol_Text), buf, buf_end, 0.0f, is_multiline ? NULL : &clip_rect);
////            m_state = (m_flags & InputTextFlags_ReadOnly) ? BND_DEFAULT : BND_ACTIVE;
////            m_carotPos = Vec2(0, -1);
////            m_renderPos = render_pos;
////        }
////        
////        if (is_multiline)
////        {
...
////        
////        m_position = position;
////        m_size = frame_bb.Max - frame_bb.Min + style.FramePadding*4;
////        
//        if (enter_pressed)
//        {
//            m_enterCallback();
//        }
//        
//        std::cout<< "Six: " << (total_bb.Max - total_bb.Min).x << "," << (total_bb.Max - total_bb.Min).y << std::endl;
//        std::cout<< "DCA: " << window->DC.CursorPos.x << "," << window->DC.CursorPos.y << std::endl;
//        
